...
```{r}
ggplot(complete, aes(date, steps)) + geom_bar(stat = "identity", colour = "light green", fill = "light green", width = 1) + facet_grid(. ~ month, scales = "free") + labs(title = "Histogram of Total # Steps Taken Each Day", x = "Date", y = "Total # Steps") + theme_bw()
```
...
* Make a time series plot (i.e. type = "l") of the 5-minute interval (x-axis) and the average # Steps taken, averaged across all days (y-axis)

```{r}
avgSteps <- aggregate(complete$steps, list(interval = as.numeric(as.character(complete$interval))), FUN = "mean")
names(avgSteps)[2] <- "meanOfSteps"

ggplot(avgSteps, aes(interval, meanOfSteps)) + geom_line(color = "light green", size = 1) + labs(title = "Time Series Plot of the 5-minute Interval", x = "5-minute intervals", y = "Average # Steps Taken") + theme_classic()
```

* Which 5-minute interval, on average across all the days in the dataset, contains the maximum # Steps?
```{r}
...
* Make a histogram of the total # Steps taken each day and Calculate and report the mean and median total # Steps taken per day. 

```{r}
ggplot(completeData, aes(date, steps)) + geom_bar(stat = "identity",
                                             colour = "light green",
                                             fill = "light green",
                                             width = 1) + facet_grid(. ~ month, scales = "free") + labs(title = "Histogram of Total # Steps Taken Each Day (complete)", x = "Date", y = "Total # Steps")+theme_bw()
```

* Do these values differ from the estimates from the first part of the assignment? What is the impact of imputing missing data on the estimates of the total daily # Steps?
...
levels(completeData$weekdays) <- list(weekday = c("Monday", "Tuesday","Wednesday","Thursday", "Friday"),
                                      weekend = c("Saturday", "Sunday"))
levels(completeData$weekdays)
table(completeData$weekdays)
```

* Make a panel plot containing a time series plot (i.e. type = "l") of the 5-minute interval (x-axis) and the average # Steps taken, averaged across all weekday days or weekend days (y-axis).
